...
function setActions(win) {
    ...
    if (shimejiStates && !shimejiStates.isDragging && !shimejiStates.isFalling) {
        if (shimejiStates.chaseCursor) timeout[win.id] = false
        if (!timeout[win.id] && (shimejiStates.state === 'idle' || shimejiStates.state === 'walk') && shimejiStates.state !== 'sit') {
            ...
            if (shimejiStates.chaseCursor) {
                ...
                timeout[win.id] = true;
                // aca utilizamos dx que seria la velocidad del personaje
                if (!shimejiStates.dx) {
                    shimejiStates.state = 'walk';
